...
function initEvents() {
    ...
    $('.content-page').scroll(initAnimationOnScroll);
...
function initAnimationOnScroll() {
    ...
    for (var i = 0; i < elementsArray.length; i++) {
        ...
        if (getWindowScroll() > needScrollValue) {
            //console.log(getWindowScroll());
            //console.log(needScrollValue);
...
function getElementTopOffset(el) {
    return el.offset().top + getWindowScroll();
}
